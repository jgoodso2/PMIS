...
namespace PMSImporter
{
    public class DataSetBuilder
    {
        ...
        public static bool Validate(DataTable inputTable, out List<string> errors)
        {
            errors = new List<string>();
            return ValidateCustomFields(inputTable, errors);
        }
        ...
        public static ProjectDataSet BuildProjectDataSetForUpdate(DataTable table, ProjectDataSet projectDataSet, 
            ProjectDataSet.ProjectRow projectRow, bool updateCustomFieldsnotrequired,bool hasPlan)
        {
            ...
            if (hasPlan)
            {
                BuildProject(inputProjectRow, projectRow, projectDataSet, projectGuid, false);
            }
            else
            {
            ...
            if (inputTaskRow.Table.Columns.Contains(_mapping.AssignmentMap["RES_NAME"]))
            {
                foreach (var assnField in _mapping.Assignment.Fields)
                {
                    ...
                    assignmentRow.TASK_UID =
                        projectDataSet.Task.First(t => t.TASK_NAME == inputTaskRow[_mapping.TaskMap["TASK_NAME"]])
                            .TASK_UID;
            ...
            ProjectDataSet projectDataSet, Guid projectGuid,bool updateCustomFieldsnotreqd)
        {
            ...
            foreach (var projectField in _mapping.Project.Fields)
            {
                ...
                if (targetColumn == "ENTERPRISE_PROJECT_TYPE_UID" || targetColumn == "ENTERPRISE_PROJECT_TYPE_NAME" || targetColumn == "PROJ_TYPE")
                {
                    continue;
                }
                Guid guidValue;
                var inputValue = inputProjectRow[sourcecolumn]; //row["Input column name in the exel file"]
            ...
            DataRow inputTaskRow, Guid projectGuid)
        {
            ...
            foreach (var customField in _mapping.Task.CustomFields)
            {
                ...
                customfieldRow.TASK_UID =
                    projectDataSet.Task.First(t => t.TASK_NAME == inputTaskRow[_mapping.TaskMap["TASK_NAME"]]).TASK_UID;
