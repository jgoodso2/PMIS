...
namespace PMSImporter
{
    public class Repository
    {
        ...
        public Guid CreateProject(ProjectDataSet projectDs)
        {
                ...
                try
                {
                    ...
                    if (WaitForQueueJobCompletion(jobGuid, Guid.NewGuid(), (int)SvcQueueSystem.QueueMsgType.ProjectUpdateTeam))
                    {
                        ...
                        else
                        {
                            ...
                            projectClient.QueueCheckInProject(Guid.NewGuid(), projUid, true, Guid.NewGuid(), "");
                        }
                ...
                if (projectDs.GetChanges(DataRowState.Added) != null)
                {
                    ...
                    if (WaitForQueueJobCompletion(jobGuid, Guid.NewGuid(), (int)SvcQueueSystem.QueueMsgType.ProjectUpdate))
                    {
                        ...
                        projectClient.QueueCheckInProject(jobGuid, projUid, true, Guid.NewGuid(), "");
                            ...
                            {
                                projectClient.QueuePublish(Guid.NewGuid(), projUid, false, "");
                                if (WaitForQueueJobCompletion(jobGuid, Guid.NewGuid(), (int)SvcQueueSystem.QueueMsgType.ProjectPublish))
                                {
        ...
        internal static bool IsProjectCheckedOut(string projectName)
        {
            var projectDS = projectClient.ReadProjectStatus(Guid.Empty, DataStoreEnum.WorkingStore, projectName, (int)PSLib.Project.ProjectType.Project);
           if (projectDS.Project.Count > 0 )
           {
               var project = projectDS.Project[0];
               return (!project.IsPROJ_CHECKOUTBYNull()) ? (project.PROJ_CHECKOUTBY != Guid.Empty) : false;
           }
           return false;
        }
    }
